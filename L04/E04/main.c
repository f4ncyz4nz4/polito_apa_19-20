...
int main() {
    Comandi codice;
    Tratta tratta[MILLE],*punt[MILLE];
    FILE *fp;
    int continua=1,N,i,ordinato=0,stamp;
    ...
    while(continua){
        ...
        switch (codice) {
            case stampa:
                ...
                for (i = 0; i < N; i++) {
                    fprintf(fp, "%s %s %s %s %s %s %d\n", punt[i]->cod, punt[i]->s_part, punt[i]->s_arr,
                            punt[i]->data, punt[i]->o_part, punt[i]->o_arr, punt[i]->rit);
                }
                break;
            case o_data:
                ordinato=Sort(punt,N,0);
                break;
            case o_tratta:
                ordinato=Sort(punt,N,1);
                break;
            case o_partenza:
                ordinato=Sort(punt,N,2);
                break;
            case o_arrivo:
                ordinato=Sort(punt,N,3);
                break;
            case ricerca:
                Ricerca(punt,N,ordinato);
                break;
            case fine:
                continua = 0;
                break;
